...
| `dev/` | Local developer helpers. `coverage.sh` runs the suite with coverage (same script CI uses); `goldens.sh` verifies or regenerates the golden references. See [docs/tools/coverage.md](../docs/tools/coverage.md) and [docs/tools/goldens.md](../docs/tools/goldens.md). |
